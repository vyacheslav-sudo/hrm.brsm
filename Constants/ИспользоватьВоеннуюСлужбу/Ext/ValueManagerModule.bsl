...
Процедура ПередЗаписью(Отказ)
	...
	Если Не ПодсистемаСуществует Тогда
		Если Значение Тогда
			ВызватьИсключение НСтр("ru='Нельзя установить значение ИспользоватьВоеннуюСлужбу';uk='Неможливо встановити значення ИспользоватьВоеннуюСлужбу'");
		КонецЕсли;
	КонецЕсли;
	...
	Если ПодсистемаСуществует Тогда
		...
		Если Значение И ИспользоватьМуниципальнуюСлужбу Тогда
			ВызватьИсключение НСтр("ru='В программе уже ведется расчет денежного содержания Муниципальных служащих, не допускается использовать одновременно расчет денежного довольствия военнослужащих и денежного содержания муниципальных служащих';uk='У програмі вже ведеться розрахунок грошового утримання муніципальних службовців, не допускається використовувати одночасно розрахунок грошового забезпечення військовослужбовців та грошового утримання муніципальних службовців'");	
		КонецЕсли;
	КонецЕсли;
