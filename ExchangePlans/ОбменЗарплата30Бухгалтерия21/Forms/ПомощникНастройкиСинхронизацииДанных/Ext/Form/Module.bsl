...
Функция Подключаемый_СтраницаДатаНачалаИспользованияОбмена_ПроверкаЗаполнения(Отказ)
	...
	Если Не ЗначениеЗаполнено(ДатаНачалаИспользованияОбмена) Тогда
		ТекстОшибки = НСтр("ru = 'Не указан месяц начала использования синхронизации.'; uk = 'Не вказано місяць початку синхронізації.'");
...
Процедура ИнициализироватьРеквизитыФормы()
	...
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	ОбменЗарплата30Бухгалтерия21.ДатаНачалаИспользованияОбмена КАК ДатаНачалаИспользованияОбмена,
	|	ОбменЗарплата30Бухгалтерия21.ИспользоватьОтборПоОрганизациям КАК ИспользоватьОтборПоОрганизациям,
	|	ОбменЗарплата30Бухгалтерия21.Организации.(
	|		Организация КАК Организация
	|	) КАК Организации,
	|	ОбменЗарплата30Бухгалтерия21.НеВыгружатьПерсональныеДанныеФизическихЛиц КАК НеВыгружатьПерсональныеДанныеФизическихЛиц
	|ИЗ
	|	ПланОбмена.ОбменЗарплата30Бухгалтерия21 КАК ОбменЗарплата30Бухгалтерия21
	|ГДЕ
	|	ОбменЗарплата30Бухгалтерия21.Ссылка = &Ссылка");
