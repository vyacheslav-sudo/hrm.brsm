...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПодробныйРасчетНачислений") Тогда
		...
		Если НЕ ТабличныйДокумент = Неопределено Тогда
			УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "ПодробныйРасчетНачислений", НСтр("ru='Подробный расчет начислений';uk='Детальний розрахунок нарахувань'"), ТабличныйДокумент,,,,Истина);
		КонецЕсли;
	КонецЕсли;
...
Функция ТабличныйДокументПодробногоРасчетаНачислений(МассивСсылок, ОбъектыПечати, ПараметрыВывода, ИмяМакета)	
	...
	МенеджерыДанныхПодробногоРасчетаНачислений = МенеджерыДанныхПодробногоРасчетаНачислений(МассивСсылок, ИмяМакета,КодЯзыкаПечать);
	...
	Для каждого ОписаниеМенеджера Из МассивОписанийМенеджеров Цикл
		...
		Для каждого Ссылка Из ОписаниеМенеджера.МассивСсылок Цикл
			...
			Для каждого ДанныеДокумента Из СтрокиДанныхДокументов Цикл
				...
				ДанныеДляПечати.Вставить("СинонимДокумента", 	МультиязычностьУкр.ПолучитьЛокализованныйСинонимОбъекта(Ссылка, КодЯзыкаПечать));
...
Процедура ЗаполнитьТабличныйДокументПодробногоРасчетаНачислений(ДанныеДляПечати, ОбъектыПечати, ИмяМакета, ТабличныйДокумент, КодЯзыкаПечать)
	...
	Для Каждого Сотрудник Из ДанныеДокумента.МассивСотрудников Цикл
		...
		ЗначенияПараметров.Вставить("НомерДокумента", 					КадровыйУчетРасширенный.НомерКадровогоПриказа(ДанныеДокумента.НомерДокумента));
		...
		ЗначенияПараметров.Вставить("ТабельныйНомер", 					ПрефиксацияОбъектовКлиентСервер.УдалитьПрефиксыИзНомераОбъекта(ДанныеСотрудника.ТабельныйНомер, Истина, Истина));
		
		ЗначенияПараметров.Вставить("Сотрудник", 						Сотрудник);
		ЗначенияПараметров.Вставить("ВидЗанятости", 					МультиязычностьУкр.ПолучитьЛокализованныйСинонимОбъекта(ДанныеСотрудника.ВидЗанятости, КодЯзыкаПечать));
		...
		Если ДанныеДляПечати.ЕстьРасчетСреднегоЗаработка Тогда
			Если Не ДанныеДляПечати.Ссылка = Неопределено Тогда 
				Если (ТипЗнч(ДанныеДляПечати.Ссылка) = Тип("ДокументСсылка.БольничныйЛист") И Не ДанныеДляПечати.Ссылка.ЯвляетсяПродолжениемБолезни) ИЛИ НЕ ТипЗнч(ДанныеДляПечати.Ссылка) = Тип("ДокументСсылка.БольничныйЛист") Тогда 
					...
					Если НЕ ИмяОбластиСреднегоЗаработка = Неопределено Тогда
						ВывестиСекциюСреднийЗаработокШапка(ТабличныйДокумент, ОбластиМакета.СекцияСреднийЗаработокШапка, НомерРаздела, ДанныеДокумента,КодЯзыкаПечать);
					...
					КонецЕсли;
				Иначе
					Если Не ДанныеДляПечати.Ссылка.ПервичныйБольничныйЛист = Неопределено Тогда 
						ТекстСообщения = НСтр("ru='Средний зароботок в сумме %1 рассчитан в документе № %2 от %3';uk='Середній заробіток у сумі %1 розрахований у документі № %2 від %3'");
						ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, ДанныеДляПечати.Ссылка.СреднийДневнойЗаработок, ДанныеДляПечати.Ссылка.ПервичныйБольничныйЛист.Номер, Формат(ДанныеДляПечати.Ссылка.ПервичныйБольничныйЛист.Дата, "ДЛФ=Д"));
		...
		Если НачисленияПоСотруднику.Количество() > 0 Тогда
			...
			Если ДанныеДляПечати.ЕстьРасчетЗарплаты Тогда
				...
				Если НачисленияСекцииПоСотруднику.Количество() > 0 Тогда
					НазваниеСекции = НСтр("ru='Расчет оплаты труда';uk='Розрахунок оплати праці'",КодЯзыкаПечать);
					ВывестиСекциюНачислений(ТабличныйДокумент, ОбластиМакета, НомерРаздела, НачисленияСекцииПоСотруднику, ДанныеДляПечати.Показатели, НазваниеСекции);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
//
Функция МенеджерыДанныхПодробногоРасчетаНачислений(МассивСсылок, ИмяМакета,КодЯзыкаПечать)
	...
	Для каждого Ссылка Из МассивСсылок Цикл
		...
		Если ОписаниеМенеджера = Неопределено Тогда
			ОписаниеДокумента = ОписаниеДокументаРасчетаНачислений(Менеджер,КодЯзыкаПечать);
...
//
Функция ОписаниеДокументаРасчетаНачислений(Менеджер,КодЯзыкаПечать)
	...
	Менеджер.ЗаполнитьОписаниеДокументаРасчетаНачислений(ОписаниеДокумента,КодЯзыкаПечать);
...
Процедура ВывестиСекциюСреднийЗаработокШапка(ТабличныйДокумент, СекцияСреднийЗаработокШапка, НомерРаздела, ДанныеДокумента,КодЯзыкаПечать)
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба")
		...
		ТекстОбласти = Строка(НомерРаздела) + ". " + НСтр("ru='Сохраняемое денежное содержание';uk='Зберігається грошове утримання'",КодЯзыкаПечать)
			+ Символы.ПС
			+ Формат(ДанныеДокумента.СохраняемоеДенежноеСодержание, "ЧДЦ=2");
			
	Иначе
		ТекстОбласти = Строка(НомерРаздела) + ". " + НСтр("ru='Расчет среднего заработка';uk='Розрахунок середнього заробітку'",КодЯзыкаПечать);
	КонецЕсли;
