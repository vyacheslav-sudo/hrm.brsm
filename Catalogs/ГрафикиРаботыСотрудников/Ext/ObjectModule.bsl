...
Процедура ЗаполнитьДанныеГрафика(ДанныеГрафика, НомерТекущегоГода) Экспорт
	...
	Пока ОбрабатываемаяДата <= ДатаОкончанияИнтервалаЗаполнения  Цикл
		...
		Если Не (УчитыватьПраздники И (ВидДня = Перечисления.ВидыДнейПроизводственногоКалендаря.Праздник ИЛИ ВидДня = Перечисления.ВидыДнейПроизводственногоКалендаря.Нерабочий))
