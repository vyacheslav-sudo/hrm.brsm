...
Процедура ВедомостьПриПолученииДанныхСтрокиСоставаНДФЛ(Форма, СтрокаСостава)
	...
	Для Каждого СтрокаНДФЛРаботника Из СтрокиНДФЛРаботника Цикл
		...
		Если Форма.ХарактерВыплаты = Перечисления.ХарактерВыплатыЗарплаты.Аванс И ЗначениеЗаполнено(СтрокаНДФЛРаботника.ДокументОснование) И ТипЗнч(СтрокаНДФЛРаботника.ДокументОснование) <> Тип("ДокументСсылка.НачислениеЗаПервуюПоловинуМесяца") Тогда	
	...
	Если ЗначениеЗаполнено(РасшифровкаОснований) Тогда
		СтрокаСостава.НДФЛРасшифровка = 
			СтрокаСостава.НДФЛРасшифровка + 
			?(ЗначениеЗаполнено(СтрокаСостава.НДФЛРасшифровка), "; "+Символы.ПС, НСтр("ru='в т.ч.';uk='в т. ч.'")+" ") + 
			РасшифровкаОснований;
	КонецЕсли;
