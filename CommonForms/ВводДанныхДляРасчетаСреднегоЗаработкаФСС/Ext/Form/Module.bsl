...
Процедура ДополнитьПредположительно()
	...
	Запрос.Текст =  
		...
		"ВЫБРАТЬ
		|	МАКСИМУМ(ПлановыйФОТ.ПериодЗаписи) КАК Период
		|ПОМЕСТИТЬ ВТМакимальныйПериод 
		|ИЗ
		|	ВТПлановыйФОТСрезПоследних КАК ПлановыйФОТ
		|;   
		|ВЫБРАТЬ
		|	НАЧАЛОПЕРИОДА(ПлановыйФОТ.Период, МЕСЯЦ) КАК Период,
		|	ПлановыйФОТ.Сотрудник.ФизическоеЛицо КАК ФизическоеЛицо,
		|	&ПорядокРасчета КАК ПорядокРасчета,
		|	СУММА(ПлановыйФОТ.ВкладВФОТ) КАК Сумма 
		|ИЗ
		|	ВТПлановыйФОТСрезПоследних КАК ПлановыйФОТ
		|ВНУТРЕННЕЕ СОЕДИНЕНИЕ
		|	ВТМакимальныйПериод КАК МаксимальныйПериод
		|ПО 
		|   ПлановыйФОТ.ПериодЗаписи = МаксимальныйПериод.Период
		|ГДЕ
		|	НЕ ПлановыйФОТ.Начисление.ВидЕСВ = ЗНАЧЕНИЕ(Перечисление.ВидыЕСВ.НеУчитывается)  
		|СГРУППИРОВАТЬ ПО
		|	НАЧАЛОПЕРИОДА(ПлановыйФОТ.Период, МЕСЯЦ),
		|	ПлановыйФОТ.Сотрудник.ФизическоеЛицо"; 
