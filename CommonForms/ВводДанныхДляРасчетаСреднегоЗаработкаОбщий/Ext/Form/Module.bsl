...
Процедура ДополнитьПредположительно()
	...
	Запрос.Текст = 
		...
		|	&Сотрудник КАК Сотрудник
		|ПОМЕСТИТЬ ВТСотрудникиПериодыНаДатуСобытия
		|ИЗ
		|	ВТПериоды КАК Периоды";
	...
	ЗарплатаКадрыОбщиеНаборыДанных.СоздатьВТИмяРегистраСрезПоследних(
		...
		ЗарплатаКадрыОбщиеНаборыДанных.ОписаниеФильтраДляСоздатьВТИмяРегистра(
			"ВТСотрудникиПериодыНаДатуСобытия",
			"Сотрудник"));
	...
	Запрос.Текст = " 
		...
		|	ВЫБОР 
		|		КОГДА НЕ(ПлановыеНачисления.Размер ЕСТЬ NULL) И НЕ(ПлановыйФОТ.ВкладВФОТ ЕСТЬ NULL)  
		...
		|
		|ГДЕ ПлановыеНачисления.Используется = ИСТИНА  
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|ИЗ
		|   ВТСотрудникиПериоды КАК Периоды
		|	ЛЕВОЕ СОЕДИНЕНИЕ ВТДанныеФОТ КАК ДанныеФОТ
		|   ПО ИСТИНА   
		|";
...
Процедура ЗаполнитьОписаниеСтрок()
	...
	НоваяСтрока.ЗаголовокСтроки = НСтр("ru='Среднемес. к-во дней';uk='Середньоміс. к-ть днів'");
	НоваяСтрока.ДобавлятьПоУмолчанию = Истина;
	НоваяСтрока.Точность = 2;
