...
Процедура СотрудникиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.Сотрудник) Тогда
		...
		ИначеЕсли Поле.Имя = "СотрудникиПредставлениеПраваНаОтпуск" Тогда
			...
			СтрокиОтпусков = Объект.ЕжегодныеОтпуска.НайтиСтроки(Новый Структура("Сотрудник", ТекущиеДанные.Сотрудник));
			Если СтрокиОтпусков.Количество() > 0 Тогда
				ПараметрыОткрытия.Вставить("ДатаОстатков", СтрокиОтпусков[0].ДатаОстатка);
			Иначе	
...
Процедура СотрудникиПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		...
		Иначе
			...
			ЗаполнитьДанныеНовойСтрокиСотрудников(Элемент.ТекущиеДанные, Объект.Подразделение, СпособРасчетаАванса, РазмерАвансаПоУмолчанию, Объект.ВидДоговора);
...
Процедура СотрудникиДолжностьПоШтатномуРасписаниюПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Элементы.Сотрудники.ТекущиеДанные.ДолжностьПоШтатномуРасписанию) Тогда
		...
		СтруктураОтбораПоСотруднику.Вставить("Сотрудник", Элементы.Сотрудники.ТекущиеДанные.Сотрудник);
		
		ЗаполнитьТолькоНачисления = Объект.ЕжегодныеОтпуска.НайтиСтроки(СтруктураОтбораПоСотруднику).Количество() <> 0;
		
		ЗаполнитьПоДолжностиПоШтатномуРасписанию(ЗаполнитьТолькоНачисления);
...
Процедура СотрудникиСотрудникПриИзмененииНаСервере(ИдентификаторСтрокиСотрудника, ТекущийСотрудник = Неопределено)
	...
	Если Объект.ЕжегодныеОтпуска.НайтиСтроки(СтруктураОтбораПоСотруднику).Количество() = 0 Тогда
		...
		ТаблицаОтпусков = ОстаткиОтпусков.ОбщиеЕжегодныеОтпуска(ТекущиеДанные.Сотрудник, Объект.ВидДоговора);
...
&НаКлиентеНаСервереБезКонтекста
Процедура ЗаполнитьДанныеНовойСтрокиСотрудников(НоваяСтрокаСотрудников, Подразделение, СпособРасчетаАванса, РазмерАвансаПоУмолчанию, ВидДоговора)
	...
	Если ВидДоговора = ПредопределенноеЗначение("Перечисление.ВидыДоговоровССотрудниками.ГигКонтракт") Тогда
	    НоваяСтрокаСотрудников.ВидЗанятости = ПредопределенноеЗначение("Перечисление.ВидыЗанятости.Совместительство");
		Если Не ЗначениеЗаполнено(НоваяСтрокаСотрудников.КатегорияЕСВ) Тогда
			НоваяСтрокаСотрудников.КатегорияЕСВ = ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Справочник.КатегорииЗастрахованныхЛицЕСВ.РаботникПоГигКонтракту");
		КонецЕсли;
	Иначе
		НоваяСтрокаСотрудников.ВидЗанятости = ПредопределенноеЗначение("Перечисление.ВидыЗанятости.ОсновноеМестоРаботы");
	КонецЕсли;
...
Процедура СотрудникиОбработкаВыбораНаСервере(ВыбранныеСотрудники)
	...
	Если СотрудникиКДобавлению.Количество() > 0 Тогда
		...
		Для каждого Сотрудник Из СотрудникиКДобавлению Цикл
			...
			ЗаполнитьДанныеНовойСтрокиСотрудников(НоваяСтрокаСотрудников, Объект.Подразделение, СпособРасчетаАванса, РазмерАвансаПоУмолчанию,Объект.ВидДоговора);
