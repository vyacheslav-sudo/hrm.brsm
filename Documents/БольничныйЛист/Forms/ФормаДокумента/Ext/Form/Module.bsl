...
Процедура УстановитьДоступностьВидаОплатыЗаСчетРаботодателя(Форма)
	...
	Если Объект.ДатаНачалаСобытия + УчетПособийСоциальногоСтрахованияКлиентСервер.КоличествоДнейЗаСчетРаботодателя(Объект.ДатаНачалаСобытия, Объект.ПричинаНетрудоспособности) * УчетПособийСоциальногоСтрахованияКлиентСервер.ДлинаСуток() > Объект.ДатаНачалаОплаты Тогда
		Доступность = Истина;
	КонецЕсли;
