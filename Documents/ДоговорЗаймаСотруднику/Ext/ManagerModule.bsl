...
Функция КарточкаДоговораЗайма(МассивОбъектов, ОбъектыПечати, ДанныеПечатиОбъектов, ПараметрыВывода)
	...
	Для Каждого ДокументСсылка Из МассивОбъектов Цикл
		...
		Если ЗначениеЗаполнено(ПоследнееИсправление) Тогда
			...
			Область.Параметры.Изменение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
										  НСтр("ru='%1 №%2 от %3';uk='%1 №%2 від %3'",КодЯзыкаПечать), 
										  МультиязычностьУкр.ПолучитьЛокализованныйСинонимОбъекта(ПоследнееИсправление.Ссылка, КодЯзыкаПечать),
										  КадровыйУчетРасширенный.НомерКадровогоПриказа(ПоследнееИсправление.Ссылка.Номер),
										  Формат(ПоследнееИсправление.Ссылка.Дата, "ДФ=dd.MM.yyyy;Л=" + КодЯзыкаПечать));
		Иначе
			Область.Параметры.Изменение = НСтр("ru='Нет';uk='Ні'",КодЯзыкаПечать);
		КонецЕсли;
