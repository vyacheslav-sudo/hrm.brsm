...
Функция ПредставлениеВозраста(ДатаРождения, ДатаАктуальности);
	...
	Иначе 
		...
		Иначе
			ВозрастШаблон = ?(Найти("12,13,14", ПоследниеДвеЦифры) = 0, НСтр("ru='%1 года';uk='%1 роки'"), НСтр("ru='%1 лет';uk='%1 років'"));
		КонецЕсли;
	КонецЕсли;
...
Функция ФотографииСотрудников(ДанныеОтчета)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ХранимыеФайлыВерсий.ДвоичныеДанныеФайла КАК ХранимыйФайл
	               |ИЗ
	               |	Справочник.Сотрудники КАК Сотрудники
	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ДвоичныеДанныеФайлов КАК ХранимыеФайлыВерсий
	               |		ПО (Сотрудники.Ссылка В (&СписокСотрудников))
	               |			И Сотрудники.ФизическоеЛицо.Фотография.ТекущаяВерсия = ХранимыеФайлыВерсий.Файл";
