...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мСоставПоказателей = "СоставПоказателей2025Кв";
...
Процедура ЗаполнитьСведенияОбОрганизации(Форма, ИзмененаДатаПодписи = Ложь)
	...
	ЗаполнитьПоказательНаСервере("FIRM_NAME", Организация.НаименованиеПолное, Форма, "ОтчетПоТруду");
	...
	ПериодОтчета = НомерКварталРимск + " квартал " + Формат(Форма.СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, "ДФ = 'гггг';Л = uk_UA");
 	ЗаполнитьПоказательНаСервере("REP_PERNM", ПериодОтчета, Форма, "ОтчетПоТруду");
	...
	КонецЕсли;
	ЗаполнитьПоказательНаСервере("FIRM_ZIP",    Индекс,    Форма, "ОтчетПоТруду");
	ЗаполнитьПоказательНаСервере("FIRM_CITY",   Город,     Форма, "ОтчетПоТруду");
	ЗаполнитьПоказательНаСервере("FIRM_OBL",    Область,   Форма, "ОтчетПоТруду");
	ЗаполнитьПоказательНаСервере("FIRM_STREET", Улица,     Форма, "ОтчетПоТруду");
	ЗаполнитьПоказательНаСервере("FIRM_DOM1",   НомерДома, Форма, "ОтчетПоТруду");
	ЗаполнитьПоказательНаСервере("FIRM_KORPUS", Корпус,    Форма, "ОтчетПоТруду");
	ЗаполнитьПоказательНаСервере("FIRM_KVART",  Офис,      Форма, "ОтчетПоТруду");
	ЗаполнитьПоказательНаСервере("FIRM_RAYON",  Район,     Форма, "ОтчетПоТруду");
	ЗаполнитьПоказательНаСервере("FIRM_TERGROM", "",       Форма, "ОтчетПоТруду");
	ЗаполнитьПоказательНаСервере("FIRM_RAYGOR",  "",       Форма, "ОтчетПоТруду");
	...
	ЗаполнитьПоказательНаСервере("ZIP_FACT",    Индекс,  Форма, "ОтчетПоТруду");
	ЗаполнитьПоказательНаСервере("CITY_FACT",   Город,   Форма, "ОтчетПоТруду");
	ЗаполнитьПоказательНаСервере("OBL_FACT",    Область, Форма, "ОтчетПоТруду");
	ЗаполнитьПоказательНаСервере("STREET_FACT", Улица,   Форма, "ОтчетПоТруду");
	ЗаполнитьПоказательНаСервере("DOM1_FACT",   НомерДома, Форма, "ОтчетПоТруду");
	ЗаполнитьПоказательНаСервере("KORPUS_FACT", Корпус,  Форма, "ОтчетПоТруду");
	ЗаполнитьПоказательНаСервере("KVART_FACT",  Офис,    Форма, "ОтчетПоТруду");
	ЗаполнитьПоказательНаСервере("RAYON_FACT",  Район,   Форма, "ОтчетПоТруду");
	ЗаполнитьПоказательНаСервере("TERGROM_FACT", "",       Форма, "ОтчетПоТруду");
	ЗаполнитьПоказательНаСервере("RAYGOR_FACT",  "",       Форма, "ОтчетПоТруду");
...
Процедура РасчетПоказателейКоличестваРаботников(МенеджерВременныхТаблиц)
	...
	Запрос.УстановитьПараметр("КонецПериода", КонецКвартала(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета));
	
	//число штатных на конец месяца	
	Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ВЫБОР КОГДА СотрудникиОрганизации.ДатаУвольнения = ДАТАВРЕМЯ(1,1,1,0,0,0) ИЛИ СотрудникиОрганизации.ДатаУвольнения > &КонецПериода  Тогда СотрудникиОрганизации.Сотрудник КОНЕЦ) КАК КоличествоСотрудников
	|ИЗ
	|	ВТСотрудникиОрганизации КАК СотрудникиОрганизации
	|ГДЕ
	|	СотрудникиОрганизации.ВидЗанятости = ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.ОсновноеМестоРаботы)";
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ВЫБОР КОГДА СотрудникиОрганизации.ДатаУвольнения = ДАТАВРЕМЯ(1,1,1,0,0,0)  ИЛИ СотрудникиОрганизации.ДатаУвольнения > &КонецПериода 
...
Процедура  РасчетФОТиПрочихВыплат(МенеджерВременныхТаблиц)
	...
	Если СтруктураРеквизитовФормы.ЗарплатаКадрыРасширенная Тогда
		ТекстЗапроса =  "ВЫБРАТЬ РАЗРЕШЕННЫЕ
						...
						|		Начисления.НачислениеУдержание.Отчет1ПВПоказателиВидовОплатыТруда = ЗНАЧЕНИЕ(Справочник.СтатьиНалоговыхДеклараций.ОтчетПоТруду_Отпуск)
						|		ТОГДА 
...
Процедура РасчетКоличестваИФОПКатегорийРаботников(МенеджерВременныхТаблиц)	
	...
	Иначе	
		...
		ПараметрыСредняяЧисленность.Вставить("ВидСредней", "ССЧ") ;
		ПараметрыСредняяЧисленность.Вставить("ОтборШтат", Ложь);
		ПараметрыСредняяЧисленность.Вставить("ОтборВнеШтат", Истина);
		ПараметрыСредняяЧисленность.Вставить("ОтборПол", Неопределено);
		ПараметрыСредняяЧисленность.Вставить("ОтборИнвалиды", Ложь);
		ПараметрыСредняяЧисленность.Вставить("ОтборНеИнвалиды", Ложь);
		ПараметрыСредняяЧисленность.Вставить("ОтборПенсионеры", Ложь);
...
Функция ПолучитьДанныеФормыНаСервере();
	...
	//2024
	КодКАТОТТГ = ПолучитьПоказательНаСервере("КАТОТТГ", ЭтаФорма, "ОтчетПоТруду");
