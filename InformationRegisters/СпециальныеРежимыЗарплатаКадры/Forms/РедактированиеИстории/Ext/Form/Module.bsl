...
Процедура КомандаОК(Команда)
	РедактированиеПериодическихСведенийКлиент.ОповеститьОЗавершении(ЭтаФорма, "СпециальныеРежимыЗарплатаКадры", ОбъектВладелец);
КонецПроцедуры
	...
	Если НЕ ОтменаРедактирования Тогда
		Если Элемент.ТекущиеДанные <> Неопределено Тогда
			Если НЕ ЗначениеЗаполнено(Элемент.ТекущиеДанные.Период) Тогда
				СообщениеОбОшибке = НСтр("ru='Необходимо указать месяц, с которого будет действовать настройка специальных режимов зарплаты и кадров';uk='Необхідно вказати місяць, з якого діятиме настройка спеціальних режимів зарплати та кадрів'");
